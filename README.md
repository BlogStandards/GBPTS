...
| 101  | [语言标签](#gbts-0101-语言标签)                         |
| 102  | [地域标签](#gbts-0102-地域标签)                         |
| 103  | [分类标签](#gbts-0103-分类标签)                         |
| 104  | [篇幅标签](#gbts-0104-篇幅标签)                         |
...
如果文章未定义语言标签，那么会从上面的结构里寻找标签，原则是就近原则，比如名为《植物图鉴》的博文位于「图鉴」栏目下，所以优先从「图鉴」栏目的标签里寻找。

因为从上到下是逐渐精确的路径，所以可以「文章」的标签可以覆盖「博客／栏目」的标签，比如博客主要使用中文语言，但文章设置的英文语言标签就能覆盖。
...
为了使标签精确，不被滥用，所以需要限制博文标签的上限，这里主要限制 GBTS 0103 分类标签。

### 草案

解析器只会记录博文标签里面的，前 10 个「分类标签」，也许这样能让作者克制，只标记文章主要的标签。

也许能与「篇幅标签」进行参与计算，对于「微型博客」可能会减少分类标签的数量，而长文应该增加数量。

比如默认情况只记录前 10 个「分类标签」，而微型博客只记录 5 个，万字长文可以增加到 15 个，还有一种可能性，8 万字以上的，像书一样多的博文大概就不需要限制数量了。
...
由于 ISO 的版权比较麻烦，[ISO 639](https://zh.wikipedia.org/wiki/ISO_639-1代码表) 的完整内容相对不是能够开箱取得的，所以还是使用开放的 [BCP 47](https://www.rfc-editor.org/info/bcp47) 的标准，这也是网站语言标签的规范。
...
根据文章中是否有地域描述，来决定是否写上地域标签，因为这是可选标签，不是必须的。
...
Telegram 的频道分类比较成熟，所以直接照搬了其分类，有少量调整。然后合并了 [ooh.directory](https://ooh.directory/) 的博客分类
...
而分类标签没有传递特性，所以博客的分类标签不会传递给博文，除非使用强制开启传递特性。
...
考虑到合理的优化「分类标签」数量，给不同篇幅的博文设立「分类标签」限制也许是可行的方法。
